"""
Image Migration Script: Contentful CDN → DigitalOcean Spaces → MongoDB Update
Restaurant: sweetgreen

Pipeline:
  MongoDB (Contentful CDN URLs) → Download → Upload to DO Spaces → Update MongoDB
...
Data structure:
  ...
  restaurant.logo                                    ← logo to migrate

Note:
  sweetgreen image URLs contain query params (?w=600&fm=avif&q=75).
  We strip these before downloading so we get the clean original file,
  and derive the correct extension from the cleaned path.
...
TARGET_RESTAURANT = "sweetgreen"

# ─── Source domains whose images should be moved to DO Spaces ────────────────
SOURCE_DOMAINS = (
    "images.ctfassets.net",   # Contentful CDN — sweetgreen dish images
    "res.cloudinary.com",     # Cloudinary — sweetgreen logo
)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("sweetgreen_migration.log"),
    ],
)
...
def strip_query_params(url: str) -> str:
    ...
    Contentful URLs look like:
      https://images.ctfassets.net/.../image.jpg?w=600&fm=avif&q=75
    We download the original without transformation params.
    """
    parsed = urlparse(url)
...
def get_extension_from_url(url: str, content_type: str = "") -> str:
    """
    Derive file extension, preferring the URL path (after stripping query params).
    Falls back to Content-Type header, then defaults to .jpg.
    """
    clean_url = strip_query_params(url)
    path = urlparse(clean_url).path
    ext  = Path(path).suffix.lower()

    # Contentful serves .avif via query param (fm=avif) but the actual
    # stored asset is usually .jpg or .png — trust the path extension.
...
def download_image(url: str) -> tuple[bytes, str]:
    """
    Download image from URL (stripping query params for Contentful URLs).
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        try:
            ...
            log.warning(f"  Download attempt {attempt}/{MAX_RETRIES} failed for {clean_url}: {e}")
...
def build_spaces_key(
    ...
    Path structure inside bucket:
      restaurants/sweetgreen/bowls/harvest_bowl/1.png
      restaurants/sweetgreen/salads/kale_caesar/1.png
    """
    return "/".join([
        DO_BASE_FOLDER,
...
def migrate_serving_infos(
    ...
    for idx, entry in enumerate(serving_infos):
        ...
        try:
            ...
            # Use slugified size as filename; all sweetgreen items have size "1"
            # so we just use the index to keep filenames unique per dish.
            size_slug = slugify(si.get("size", "default"))
            filename  = f"{size_slug}_{idx}{ext}" if idx > 0 else f"{size_slug}{ext}"
...
def migrate_restaurant(restaurant_doc: dict, dry_run: bool = False) -> list[UpdateOne]:
    ...
    for cat in categories:
        ...
        # Future-proof: handle subCategories if they ever appear
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Migrate sweetgreen images: Contentful CDN → DigitalOcean Spaces"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
